...
# --- MathEvals addition (not in the reference task) -----------------------
# Same as process_docs above, but first restrict to the modern Putnam era
# (year >= 1985) and put the rows in (year, id) order, so a `--limit N` run
# lands on the first N *modern* problems. Nothing else changes: the prompt,
# the 4-shot examples, extraction and is_equiv grading are all untouched.
MODERN_MIN_YEAR = 1985


def process_docs_modern(dataset: datasets.Dataset) -> datasets.Dataset:
    def _year(doc: dict) -> int:
        try:
            return int(str(doc.get("year", "")).strip())
        except (TypeError, ValueError):
            return -1

    rows = sorted(
        (dict(d) for d in dataset if _year(d) >= MODERN_MIN_YEAR),
        key=lambda d: (_year(d), str(d.get("id", ""))),
    )

    def _mk(doc: dict) -> dict:
        return {
            "problem": doc["problem"],
            "solution": doc["solution"],
            "answer": normalize_final_answer(
    ...
    return datasets.Dataset.from_list([_mk(d) for d in rows])
